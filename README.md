...
- 2026/05/20：增强用户输入校验，杜绝了非法输入；优化系统环境信恨的展示逻辑，确保在不同系统版本下均能稳定运行。
- 2026/05/31：修复部分bug。
## 📖 项目介绍
# 🚀 Xray 一键安装脚本
...
- 推荐Vultr，可随时换vps换IP，按时计费。(https://www.vultr.com/?ref=6999923)  
...
| **VLESS-WS-TLS** | **CDN 兼容/高通用性**：采用标准 WebSocket 传输，搭酌 TLS 加密可完美接入 Cloudflare 等 CDN 隐藏眜实 IP；通过 443 端口反代，适合在严寂防火墙环境下生存。 |
| **VLESS-gRPC-TLS** | **低延迟/多路复用**：利用 gRPC 协议的长连接特性，显著提升在高丢包网络环境下的稳定性；原生支持多路复用，握手速度快，适合对响应速度有要求的移动端场景。 |
| **VLESS-XHTTP-TLS** | **流式传输/防指纹**：Xray 核心新推出的传输方式，模拟真实浏览器的 HTTP 请求行为；相比 WebSocket 拥有更轻量级的封装，具备更现代化的防指纹识别机制。 |
...
| **VMess-WS-TLS** | **广泛兼容/传统方案**：最经典的抗封锁方案之一，几乎所存在的客户端均能完美支持；适合作为保底节点，搭酌 CDN 转发可有效拯救被封锁的服务器 IP。 |
